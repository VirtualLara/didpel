...
import { listarProductosMiCiudad, listarProductosPorCategoriaCiudad, listarNotificacionesPendientes } from '../../Utils/Acciones';
...
export default function TiendaMiCiudad() {
  ...
  useFocusEffect(
    useCallback(() => {
      (async () => {
        setNotificaciones(0)
        setList(await listarProductosMiCiudad());
  ...
  const cargarFiltroPorCategoria = async (categoria) => {
    ...
    const listaProductosCategoria = await listarProductosPorCategoriaCiudad(categoria);
